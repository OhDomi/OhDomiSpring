MERGE INTO app_users (user_id, login_id, password_hash, name, role, phone, active) KEY(user_id) VALUES
  (1, 'admin', '{noop}1234', '본사 운영팀', 'ADMIN', '02-1234-5678', TRUE),
  (2, 'demo', '{noop}1234', '김도윤', 'OWNER', '010-4820-1593', TRUE),
  (3, 'seongsu', '{noop}1234', '이서준', 'OWNER', '010-7412-8850', TRUE),
  (4, 'jamsil', '{noop}1234', '박지우', 'OWNER', '010-3387-2140', TRUE),
  (5, 'yeouido', '{noop}1234', '최하늘', 'OWNER', '010-9061-3724', TRUE),
  (6, 'busan', '{noop}1234', '정민호', 'OWNER', '010-6112-4409', TRUE);

MERGE INTO stores (store_id, owner_user_id, store_code, name, region, address, phone, open_time, close_time, operation_status, opened_on, contract_ends_on, monthly_sales_target) KEY(store_id) VALUES
...
MERGE INTO staff_shifts (staff_shift_id, store_id, staff_name, staff_role, work_date, starts_at, ends_at, status) KEY(staff_shift_id) VALUES
...
MERGE INTO facilities (facility_id, store_id, name, active) KEY(facility_id) VALUES
  (1, 1, '냉장고', TRUE), (2, 1, 'POS 기기', TRUE), (3, 1, '조리대', TRUE), (4, 1, '에어컨', TRUE);
MERGE INTO facility_checks (facility_check_id, facility_id, status, memo, checked_at) KEY(facility_check_id) VALUES
  (1, 1, 'WARNING', '온도 변동이 감지되어 재확인이 필요합니다.', CURRENT_TIMESTAMP),
  (2, 2, 'NORMAL', '결제 및 주문 연동 정상', CURRENT_TIMESTAMP),
  (3, 3, 'CHECK_REQUIRED', '위생 점검 사진 재업로드가 필요합니다.', DATEADD('HOUR', -15, CURRENT_TIMESTAMP)),
  (4, 4, 'NORMAL', '이상 없음', CURRENT_TIMESTAMP);

MERGE INTO menu_items (menu_item_id, name, category, price, active) KEY(menu_item_id) VALUES
  (1, '연어 포케', '시그니처', 11000, TRUE), (2, '참치 마요 덮밥', '덮밥', 9000, TRUE),
  (3, '메밀 소바', '면류', 9000, TRUE), (4, '닭가슴살 샐러드', '샐러드', 11000, TRUE);

MERGE INTO customer_orders (customer_order_id, store_id, channel, ordered_at, total_amount, status) KEY(customer_order_id) VALUES
  (1, 1, 'IN_STORE', DATEADD('HOUR', -4, CURRENT_TIMESTAMP), 704000, 'COMPLETED'),
  (2, 1, 'DELIVERY', DATEADD('HOUR', -3, CURRENT_TIMESTAMP), 459000, 'COMPLETED'),
  (3, 1, 'TAKEOUT', DATEADD('HOUR', -2, CURRENT_TIMESTAMP), 417000, 'COMPLETED'),
  (4, 2, 'IN_STORE', DATEADD('HOUR', -2, CURRENT_TIMESTAMP), 5140000, 'COMPLETED'),
  (5, 3, 'DELIVERY', DATEADD('HOUR', -2, CURRENT_TIMESTAMP), 3760000, 'COMPLETED'),
  (6, 4, 'IN_STORE', DATEADD('HOUR', -2, CURRENT_TIMESTAMP), 4250000, 'COMPLETED'),
  (7, 5, 'DELIVERY', DATEADD('HOUR', -2, CURRENT_TIMESTAMP), 2980000, 'COMPLETED');
MERGE INTO customer_order_items (customer_order_item_id, customer_order_id, menu_item_id, quantity, unit_price) KEY(customer_order_item_id) VALUES
  (1, 1, 1, 64, 11000), (2, 2, 2, 51, 9000), (3, 3, 3, 37, 9000),
  (4, 3, 4, 8, 10500);

MERGE INTO inventory_items (inventory_item_id, store_id, item_name, category, unit, current_quantity, reorder_level, unit_price) KEY(inventory_item_id) VALUES
...
MERGE INTO order_recommendations (recommendation_id, inventory_item_id, recommendation_date, expected_usage, recommended_quantity, risk_level, reason) KEY(recommendation_id) VALUES
...
MERGE INTO purchase_orders (purchase_order_id, store_id, order_number, status, ordered_at, total_amount, created_at) KEY(purchase_order_id) VALUES
...
MERGE INTO purchase_order_items (purchase_order_item_id, purchase_order_id, inventory_item_id, quantity, unit_price) KEY(purchase_order_item_id) VALUES
  (1, 1, 1, 16, 18000), (2, 1, 2, 10, 3500), (3, 1, 3, 70, 750), (4, 1, 4, 5, 10600);

MERGE INTO hygiene_inspections (inspection_id, store_id, score, status, reviewer, summary, inspected_at) KEY(inspection_id) VALUES
...
MERGE INTO hygiene_check_results (check_result_id, inspection_id, item_name, score, status, memo) KEY(check_result_id) VALUES
...
MERGE INTO hygiene_images (image_id, inspection_id, image_url, category, analysis_result, uploaded_at) KEY(image_id) VALUES
...
MERGE INTO improvement_tasks (improvement_task_id, inspection_id, title, description, priority, status) KEY(improvement_task_id) VALUES
  (1, 1, '출입구 주변 박스 정리', '홀 입구 근처 적재물로 이동 동선에 방해될 수 있습니다.', 'WARNING', 'OPEN'),
  (2, 1, '바닥 물기 재확인', '점심 피크 전 바닥 미끄럼 위험 여부를 확인하세요.', 'CHECK', 'OPEN');

MERGE INTO risk_assessments (risk_assessment_id, store_id, risk_score, risk_level, sales_change_rate, hygiene_score, delayed_order_count, complaint_count, main_reason, prediction, recommended_action, assessed_at) KEY(risk_assessment_id) VALUES
...
MERGE INTO board_posts (post_id, author_user_id, store_id, board_type, category, title, content, status, is_pinned, is_urgent, view_count, created_at, updated_at) KEY(post_id) VALUES
...
MERGE INTO board_answers (answer_id, post_id, author_user_id, content, created_at, updated_at) KEY(answer_id) VALUES
  (1, 4, 1, 'AI 발주 추천 수량은 최근 판매량과 내일 예상 주문 수를 기준으로 산정됩니다.', '2026-07-21 13:00:00', '2026-07-21 13:00:00');

MERGE INTO notifications (notification_id, store_id, recipient_user_id, level, title, content, is_read, created_at) KEY(notification_id) VALUES
  (1, 1, 1, 'URGENT', '강남역점 위생 점검 필요', 'AI 사진 분석에서 조리대 오염 가능성이 감지되었습니다.', FALSE, DATEADD('MINUTE', -10, CURRENT_TIMESTAMP)),
  (2, 1, 2, 'WARNING', '연어 재고 부족 예상', '예상 사용량 대비 현재 재고가 부족합니다.', FALSE, DATEADD('MINUTE', -32, CURRENT_TIMESTAMP)),
  (3, NULL, 1, 'INFO', '주간 매출 리포트 도착', '전주 대비 전체 매출이 증가했습니다.', FALSE, DATEADD('HOUR', -1, CURRENT_TIMESTAMP));
